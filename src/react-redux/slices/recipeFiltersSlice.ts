...
const recipeFiltersSlice = createSlice({
    ...
    initialState,
    reducers: {
        setIncludeIngredients(state, action: PayloadAction<IngredientVariantDTO[]>) {
            state.includeIngredients = action.payload;
        },
        setExcludeIngredients(state, action: PayloadAction<IngredientVariantDTO[]>) {
...
export const { setIncludeIngredients, setExcludeIngredients, resetFilters } = recipeFiltersSlice.actions;
