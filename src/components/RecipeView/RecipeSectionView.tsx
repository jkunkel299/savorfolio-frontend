...
export default function RecipeSectionView({
  ...
  const unsectionedIngredients = ingredients!.filter(
    (ing: IngredientEntry) => !ing.sectionName
  );
  const unsectionedInstructions = instructions!.filter(
    (ins: InstructionEntry) => !ins.sectionName
  );
  const ingHasSections =
    recipeSections!.length > 0 && ingredients!.some((ing) => ing.sectionName);
  const insHasSections =
    recipeSections!.length > 0 && instructions!.some((ins) => ins.sectionName);
